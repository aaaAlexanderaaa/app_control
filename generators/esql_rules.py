...
def build_prefilter_regex(apps: list[dict]) -> str | None:
    terms = build_prefilter_terms(apps)
    if not terms:
        return None
...
def generate_optimized_esql(
    ...
    *,
    ...
    prefilter_regex = build_prefilter_regex(apps)
    if prefilter_regex:
        escaped_regex = escape_esql(prefilter_regex)
        lines.append("")
        lines.append(
            f'| WHERE observed_domain RLIKE ".*({escaped_regex}).*" OR apps_lower RLIKE ".*({escaped_regex}).*"'
        )
